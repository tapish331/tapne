...
export default function LoginModal({ open, onOpenChange, onSuccess }: LoginModalProps) {
  ...
  const hasGoogle = Boolean(googleOAuthUrl);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    setLoading(true);
    setError("");
    const ok = mode === "login" ? await login(identifier, password) : await signup(name, identifier, password);
    setLoading(false);
  ...
  const description = mode === "login" ? "Log in to continue" : "Start your journey with Tapne";
