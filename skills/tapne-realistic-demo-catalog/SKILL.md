...
Seed a realistic, maximum-traffic Tapne marketplace with ~70 demo users, ~70 trips, ~35 blogs, and ~1,300 social/activity records — all tagged `is_demo=True` — then control their public visibility with a single env-var flip.
...
### Step 2 — Audit all seeding entry points

Before seeding, confirm every model that accepts demo data is covered. Run:
...
If any import fails, the model may have been renamed or moved. Fix the import in `populate_demo_catalog.py` before proceeding.

### Step 3 — Seed with maximum-traffic data
...
Expected: trips ≥ 65, blogs ≥ 33, profiles ≥ 70.
...
Restart the server. Run Gate C checks (see TROUBLESHOOTING.md).
...
`--reset` deletes ALL `is_demo=True` rows before re-seeding. See Hard Rule 4 before running on any shared DB.
...
4. **Re-running the command without `--reset` is always safe.** All creates use `update_or_create` / `get_or_create` — no duplicate rows.
...
9. **Declare the run done only after all acceptance gates in TROUBLESHOOTING.md pass.** Partial gate failures indicate migration, seeding, or queryset-patch gaps.
...
| [TROUBLESHOOTING.md](TROUBLESHOOTING.md) | Gap-fix protocol, acceptance gate checklists A–F |
